...
with open('C:\\Users\\Nawaz\\Desktop\\qadev\\js tensorflow\\testfilenew','rb') as f:
    transData=f.read()

allTransData = json.loads(transData)

simpleTarget = list(range(0, datalength))
# Adding all translations to allTrans


for i in range(0, int(len(allTransData)/datalength)):
    allTransTarget = allTransTarget + simpleTarget
...
#model.add(hub_layer)
model.add(tf.keras.layers.Dense(320, activation='relu', input_shape=[512]))
...
model.fit(train_dataset, epochs=50, validation_data=validation_dataset, verbose=1, callbacks=early_stop)
